...
def playMatches(player1, player2, EPISODES, logger, turns_until_tau0, memory = None, goes_first = 0):
    ...
    for e in range(EPISODES):
        print('EPISODE', str(e+1),'of',str(EPISODES))
        # logger.info('====================')
        # logger.info('EPISODE %d OF %d', e+1, EPISODES)
        # logger.info('====================')
        print('episode time {}'.format(str(time.time()-last_t)))
        last_t=time.time()
        state = env.reset()
        ...
        if player1Starts == 1:
            ...
            # logger.info(player1.name + ' plays as Cho')
        else:
            players = {1:{"agent": player2, "name":player2.name}
                    , -1: {"agent": player1, "name":player1.name}
                    }
            # logger.info(player2.name + ' plays as Han')
            # logger.info('--------------')
            ...
            if done == 1: 
                ...
                if memory != None:
                    ...
                    for move in memory.stmemory:
                        move['value'] =move['playerTurn']*value
                       
                    memory.commit_ltmemory()
             
                if value == 1:
                    # logger.info('%s WINS!', players[state.playerTurn]['name'])
                    print(players[state.playerTurn]['name'],' wins')
                ...
                points[players[state.playerTurn]['name']].append(value*state.playerTurn)
                points[players[-state.playerTurn]['name']].append(value*(-state.playerTurn))
